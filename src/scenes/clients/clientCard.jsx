            ...
            <Grid container>
                <Grid item xs={12}>
                    ...
                    >
                        ...
                        <Box>
                            <Typography variant="h5" width={"95%"} color={theme.palette.blue.first} fontWeight="bold" gutterBottom onClick={() => copyToClipboard(`${data?.FIRSTNAME || ''} ${data?.NAME2 || ''}`)}>
                        ...
                        <Grid container alignItems={"center"}>
                            <Grid item xs={12} sm={12} md={4} lg={4} xl={4}>
                                <Box className="photo-box">
                                    {data?.TYPECUST2 ?
                                        <img
                                            src={TypeClient?.includes(data?.TYPECUST2) ? (require('../../assets/profileColored.png')) : require('../../assets/companyColored.png')}
                                            alt={TypeClient?.includes(data?.TYPECUST2) ? "Profile" : "Société"}
                            ...
                            <Grid item xs={12} sm={12} md={8} lg={8} xl={8}>
                                <Box>
                                    {TypeClient?.includes(data?.TYPECUST2) &&
                                        <>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={12} sm={12} md={8} lg={8} xl={8}>
                                <Box gap={1} sx={{ pt: 2, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
                                    {(data?.PHONE || data?.PHONEPRI) &&
                                        <Typography
                                            title="Tél"
                                            component={"a"}
                                            href={`tel:${data.PHONE}`}
                                            sx={{ display: 'flex', alignItems: 'center', color: theme.palette.blue.first }}
                                            onClick={() => copyToClipboard(`${(data?.PHONE && data?.PHONEPRI) ? `${data?.PHONE} / ${data?.PHONEPRI}` : (data?.PHONE ? data?.PHONE : (data?.PHONEPRI ? data?.PHONEPRI : '-'))}`)}
                                        >
                                            <PhoneIcon sx={{ marginRight: '.7rem' }} />
                                            <span style={{ color: "grey" }}>{(data?.PHONE && data?.PHONEPRI) ? `${data?.PHONE} / ${data?.PHONEPRI}` : (data?.PHONE ? data?.PHONE : (data?.PHONEPRI ? data?.PHONEPRI : '-'))}</span>
                                        </Typography>
                                    }
                                    {data?.FAX &&
                                        <Typography
                                        ...
                                        >
                                            ...
                                            <span style={{ color: "grey" }}>{data?.FAX}</span>
                                        </Typography>
                                    }
                                    {data?.EMAIL &&
                                        <Typography
                                    ...
                                    {data?.ADDRESS &&
                                        <Typography title="Adresse" sx={{ display: 'flex', alignItems: 'center', color: theme.palette.blue.first }} onClick={() => copyToClipboard(`${data.ADDRESS}`)}>
                                            ...
                                            <span style={{ color: "grey" }}>{data?.ADDRESS}</span>
                                        </Typography>
                                    }
                                </Box>
                            </Grid>
                            <Grid item xs={12} sm={12} md={4} lg={4} xl={4}>
                                <Box gap={2} sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end', height: '100%' }}>
                                    <Box sx={{
                                        width: '100%',
                                        mt: {
                                            xs: 3
